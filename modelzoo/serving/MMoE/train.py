...
ITEM_INPUTS = list(set(ALL_FEATURE_COLUMNS) - set(USER_INPUTS))
...
class MMOE():
    def __init__(self,
        ...
        if self.is_structured:
            user_tensors = [ self._inputs[name] for name in USER_INPUTS ]
            item_tensors = [ self._inputs[name] for name in ITEM_INPUTS ]
...
def main(tf_config=None, server=None):
    ...
    if is_saved_model:
        ...
        for name in IDENTITY_INPUTS:
            inputs[name] = tf.placeholder(tf.int32, [None], name=name)
        for name in HASH_INPUTS:
            inputs[name] = tf.placeholder(tf.string, [None], name=name)
        real_input = inputs

    # create model
    model = MMOE(input=real_input,
