...
from tensorflow.contrib.structured_model import enable_structured_model_mode_v2, get_structured_model, create_tile_ops, create_split_ops
...
class DIEN():
    def __init__(self,
        ...
        if is_saved_model:
            # use copy to avoid change the reference of input placeholder when the features are transformed
            self._inputs = inputs.copy()
            self._feature = self._inputs
            if is_structured:
                assert(item_packsize is not None)
                self.item_packsize = item_packsize
        else:
            self._feature = inputs[0]
            self._label = inputs[1]
            
        self.is_saved_model = is_saved_model
        ...
        self.is_training = not self.is_saved_model
        ...
        self._batch_size = tf.shape(self._feature['UID'])[0] #args.batch_size
        ...
        #     user_tensors = [ self._inputs[name] for name in USER_COLUMNS ]
        #     item_tensors = [ self._inputs[name] for name in ITEM_COLUMNS ]
            #enable_structured_model_mode_v2(user_tensors, item_tensors, ITEM_PACKSIZE)
            #get_structured_model().graph_transform()
        if self.is_training:
    ...
    def _create_model(self):
        ...
        with tf.variable_scope('rnn_1'):
            ...
            print('Transformed FLOPS: {}'.format(flops.total_float_ops))
            # Create tile ops
            if self.is_structured:
                run_output_1 = create_tile_ops(run_output_1, self.item_packsize)
                his_item_emb = create_tile_ops(his_item_emb, self.item_packsize)
                noclk_his_item_emb = create_tile_ops(noclk_his_item_emb, self.item_packsize)
                sequence_length = create_tile_ops(sequence_length, self.item_packsize)
            mask = tf.sequence_mask(sequence_length)
        # Aux loss
        ...
        # Create tiled ops
        if self.is_structured:
            uid_emb_tiled = create_tile_ops(uid_emb, self.item_packsize)
            item_his_eb_sum_tiled = create_tile_ops(item_his_eb_sum, self.item_packsize)

            top_input = tf.concat([
                uid_emb_tiled, item_emb, item_his_eb_sum_tiled, item_emb * item_his_eb_sum_tiled,
                final_state2
            ], 1)
        else:
            top_input = tf.concat([
...
def main(tf_config=None, server=None):
    ...
    if args.saved_model_path:
        is_saved_model = True
        inputs = {}
        inputs['UID'] = tf.placeholder(tf.string, [None], name='UID')
        inputs['ITEM'] = tf.placeholder(tf.string, [None], name='ITEM')
        inputs['CATEGORY'] = tf.placeholder(tf.string, [None], name='CATEGORY')
        inputs['HISTORY_ITEM'] = tf.placeholder(tf.string, [None], name='HISTORY_ITEM')
        inputs['HISTORY_CATEGORY'] = tf.placeholder(tf.string, [None], name='HISTORY_CATEGORY')
        inputs['NOCLK_HISTORY_ITEM'] = tf.placeholder(tf.string, [None], name='NOCLK_HISTORY_ITEM')
        inputs['NOCLK_HISTORY_CATEGORY'] = tf.placeholder(tf.string, [None], name='NOCLK_HISTORY_CATEGORY')
        real_input = inputs
        real_input = inputs
    ...
    model = DIEN(feature_column=feature_column,
                 ...
                 is_structured=args.structured,
                 item_packsize=args.item_packsize)
    ...
    if is_saved_model:
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            if False:
                export_dir = args.saved_model_path + '/' + str(args.model_version)  # './savedmodel'
                print('Exporting trained model to', export_dir)
                tf.saved_model.simple_save(
                    sess,
                    export_dir,
                    inputs=inputs,
                    outputs={"predict": model.probability},
                    legacy_init_op=tf.tables_initializer())
                tf.logging.info("PB Model is saved!")
    else:
...
def get_arg_parser():
    ...
    parser.add_argument('--saved_model_path',
                        help='Full path to saved model exporting directory',
                        default=None)
    ...
    parser.add_argument('--structured',
                        help='Enable structured model for serving',
                        type=bool,
                        default=False)
    parser.add_argument('--item_packsize',
                        help='package size for structured model serving',
                        type=int,
                        default=None)
