...
print(len(FEATURE_COLUMNS), len(ITEM_COLUMN))
...
class DLRM():
    ...
    def __init__(self,
        ...
        if self.is_structured:
            ...
            user_tensors = [ self._inputs[name] for name in USER_COLUMNS + HISTORY_COLUMN + USER_TZ_COLUMN ]
            item_tensors = [ self._inputs[name] for name in ITEM_COLUMN ]
...
def main(tf_config=None, server=None):
    ...
    is_saved_model = args.saved_model_path
    real_input = next_element
    if is_saved_model:
        inputs = {}
        for name in CONTINUOUS_COLUMNS:
            inputs[name] = tf.placeholder(tf.float32, [None], name=name)
        string_input_names = list(set(FEATURE_COLUMNS) - set(CONTINUOUS_COLUMNS))
        for name in string_input_names:
